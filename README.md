...
A production-ready course marketplace booking system with comprehensive observability and instrumentation.

## About this project

This project is a gRPC server, written in Go, that provides endpoints to manage courses catalog and bookings for an online course marketplace. This project uses library named [gRPC-gateway](https://github.com/grpc-ecosystem/grpc-gateway) which provides a RESTful API endpoints proxy. So, while the implementation is actually a gRPC server, you can also access it by using `curl` or any other way you used to.

## Key Features

### Core Functionality

- **Course Catalog Management** - Browse and search through course offerings with pagination support
- **Batch Management** - Multiple scheduled sessions per course with seat availability tracking
- **Booking System** - Complete reservation workflow with time-based holds (10-minute expiration)
- **Payment Processing** - Handle payment completion, failures, and status tracking
- **Customer Management** - Store and manage customer information with bookings

### Technical Highlights

- **Dual Protocol Support** - Native gRPC with automatic REST API via gRPC-Gateway
- **Optimistic Locking** - Version-based concurrency control for seat reservations
...
- **Load Testing** - Locust-based load generator with multiple user scenarios
- **Seed Data** - Automated database seeding with 1,000 sample courses
